...
class SnakeModel {
  List<int> _body = [];
  late Direction _moveDirection;

  SnakeModel.getSnake({int length = 5}) {
    Random random = new Random();

    // Get random direction to start moving
    int index = random.nextInt(Direction.values.length);
    this._moveDirection = Direction.values[index];
    ...
    // Fill snake body
    int bodyCoeff = _getBodyCoeff(this._moveDirection);
  ...
  int _getBodyCoeff(Direction direction) {
    switch (direction) {
      case Direction.up: return fieldWidth;
      case Direction.down: return -fieldWidth;
